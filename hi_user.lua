...
local function keko_SNIEBR(data)
    ...
    if not datahmade:get("SNIEBR:get:my:frind:gr:"..bot_id..msg.chat_id_) then 
    ...
    send(msg.chat_id_, msg.id_, 1, "🦁┇ الزمال😂 : @["..user_ts.."]\n📪┇يصيحك : @["..keko_ts[1].."]", 1, 'html')
    end
    tdcli_function ({
    ID = "GetChatMember",
    ...
    resolve_username(keko_ts[1],SNIEBR_get)
    end
    ...
    datahmade:set("SNIEBR:get:my:frind:gr:"..bot_id..msg.chat_id_,"SNIEBR")
    send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعطيل صيح", 1, 'html')
    end 
    if text and text == "تفعيل صيح" then 
    datahmade:del("SNIEBR:get:my:frind:gr:"..bot_id..msg.chat_id_)
