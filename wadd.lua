...
local function keko_SNIEBR(data)
    ...
    datahmade:set("SNIEBR:loock:add:w:"..msg.chat_id_..bot_id,"SNIEBR")
    send(msg.chat_id_, msg.id_, 1, "🔘┇ تم تعطيل خاصيه ضافني", 1, 'html')
    return "keko"
    end 
    if text and text == "تفعيل ضافني" then 
    datahmade:del("SNIEBR:loock:add:w:"..msg.chat_id_..bot_id)
    send(msg.chat_id_, msg.id_, 1, "☑️┇ تم تفعيل خاصيه ضافني", 1, 'html')
    return "keko"
    end 
    end
    if not datahmade:get("SNIEBR:loock:add:w:"..msg.chat_id_..bot_id) then 
    if msg.content_.ID == "MessageChatAddMembers" then
    datahmade:set("SNIEBR:add:me:w:"..bot_id..msg.chat_id_..msg.content_.members_[0].id_,msg.sender_user_id_)    
    end 
    if text and text:match("(.*)(ضافني)(.*)") then 
    if_keko =  datahmade:get("SNIEBR:add:me:w:"..bot_id..msg.chat_id_..msg.sender_user_id_)    
    if not if_keko then 
    send(msg.chat_id_, msg.id_, 1, '📮┇انته دخلت عل رابط لتلح', 1, "html")
    else
    local id = if_keko
    local text = '📮┇هاذ هاذ الضافك والله 😂😔 .'
